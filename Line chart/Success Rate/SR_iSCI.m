%% Assumed existing variables (each is a 14 × 8 double matrix)
% off, risk, vision, smell, vision_smell

data_all = {off, risk, vision, smell, vision_smell};
groupNames = {'MESC-off','MESC-on','MESC-on (VD)','MESC-on (OD)','MESC-on (VD+OD)'};

%% RGB colormap (5 distinct colors)
colors = lines(5);
...
%% Plot mean ± 95% CI for each condition
for i = 1:5
    data = data_all{i};          % Current dataset (14 × 8)
    mu   = mean(data, 2);        % Mean success rate
